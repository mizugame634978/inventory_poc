...
- **特定環境の絶対パス(ユーザ名入りのフルパス等)をドキュメント/コードに書かない**。
  相対パスかプレースホルダ(例 `<inventory_poc までのパス>`)で表現する。環境依存を避けるため。

## memory 方針(対象: 個人開発者)

- memory は**有効のまま使う**(自動蓄積・関連想起という、CLAUDE.md にはない価値を活かす)。
- 使い分け:
  - 共有・再現に効く知識 → **in-repo**(CLAUDE.md / docs / issues)。コミットしてレビュー対象に。
  - 個人的・環境的・カジュアルな運用知/好み → **memory** に任せる。
  - memory 内で durable かつ共有価値が出たものは **docs へ昇格**する。
- 起動は `inventory_poc` 固定。memory がこのプロジェクト専用キーになり、兄弟プロジェクトと隔離される
  (詳細は `docs/claude-code-settings-scope.md`)。
